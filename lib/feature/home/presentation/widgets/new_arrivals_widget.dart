...
class NewArrivalsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HomeCubit, HomeState>(
      buildWhen:
          (previous, current) =>
              current is NewArrivalsLoadedState ||
              current is NewArrivalsLoadingState ||
              current is NewArrivalsErrorState,
      builder: (context, state) {
        if (state is NewArrivalsLoadedState) {
          var books =
              context.read<HomeCubit>().newArrivalsResponse?.data?.products ??
              [];
          return Padding(
            ...
            child: Column(
              ...
              children: [
                Text('New Arrivals', style: getTitleTextStyle(context)),
                const Gap(10),
                GridView.builder(
